...
@SmokeTest1
  ...
  Scenario Outline: Verify User able to Create new Employee
    ...
    * def generateTestData = call read(filePath.GenerateTestData+'@generateEmpData'){fileName:'#(testDataFilePath)', region: '<region>', index: '<index>'}
    * def data = read('classpath:'+filePath.EmpTestData)
    * print data
    * def request_body = read('classpath:'+ filePath.EmpPostRBody)
    * print request_body
    ...
    * def updateTestData = call read(filePath.GenerateTestData+'@updateEmpId'){fileName:'#(testDataFilePath)', region: '<region>', index: '<index>', empId:'#(empId)' }
    * print empId
